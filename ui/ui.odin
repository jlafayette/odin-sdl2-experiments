...
main :: proc() {
    ...
    if err := sdl2.UpdateTexture(atlas_texture, nil, raw_data(pixels), 4*mu.DEFAULT_ATLAS_WIDTH); err != 0 {
            ...
            #partial switch event.type {
                ...
                case .MOUSEBUTTONDOWN, .MOUSEBUTTONUP:
                    ...
                    switch event.button.button {
                        case sdl2.BUTTON_LEFT: fn(ctx, event.button.x, event.button.y, .LEFT)
                        case sdl2.BUTTON_MIDDLE: fn(ctx, event.button.x, event.button.y, .MIDDLE)
                        case sdl2.BUTTON_RIGHT: fn(ctx, event.button.x, event.button.y, .RIGHT)
                    }
                case .KEYDOWN, .KEYUP:
...
render :: proc(ctx: ^mu.Context, renderer: ^sdl2.Renderer, atlas_texture: ^sdl2.Texture, bg: mu.Color) {
    render_texture :: proc(
